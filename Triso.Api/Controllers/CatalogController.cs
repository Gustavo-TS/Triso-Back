...
{
    [HttpGet("products")]
    public async Task<IActionResult> Products([FromQuery] string? q, [FromQuery] string? category, [FromQuery] int limit = 20, CancellationToken ct = default) { var query = db.Products.AsNoTracking().Where(x => x.Status == "published").Include(x => x.Category).Include(x => x.Images).Include(x => x.MarketplaceLinks).ThenInclude(x => x.Marketplace).AsSplitQuery().AsQueryable(); if (!string.IsNullOrWhiteSpace(q)) query = query.Where(x => EF.Functions.ILike(x.Name, $"%{q.Trim()}%") || EF.Functions.ILike(x.Description ?? "", $"%{q.Trim()}%")); if (!string.IsNullOrWhiteSpace(category)) query = query.Where(x => x.Category.Slug == category); var items = await query.OrderByDescending(x => x.CreatedAt).Take(Math.Clamp(limit, 1, 50)).ToListAsync(ct); return Ok(new { data = items.Select(Map) }); }
    [HttpGet("products/{slug}")] public async Task<IActionResult> Product(string slug, CancellationToken ct) { var value = await db.Products.AsNoTracking().Where(x => x.Status == "published" && x.Slug == slug).Include(x => x.Category).Include(x => x.Images).Include(x => x.MarketplaceLinks).ThenInclude(x => x.Marketplace).AsSplitQuery().SingleOrDefaultAsync(ct); return value is null ? NotFound() : Ok(new { data = Map(value) }); }
